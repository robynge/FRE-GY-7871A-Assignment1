...
PARSER_VERSION = "2"
_VISIBLE_IX_TAGS = {"ix:nonnumeric", "ix:nonfraction", "ix:fraction", "ix:continuation", "ix:exclude"}
...
def html_to_text(raw_html: str, drop_numeric_tables: bool = True) -> str:
    ...
    for tag in list(soup.find_all(True)):
        ...
        style = re.sub(r"\s+", "", tag.attrs.get("style") or "").lower()
        if "display:none" in style or "visibility:hidden" in style or tag.has_attr("hidden"):
            tag.decompose()
            continue
        if name in _VISIBLE_IX_TAGS:
            # These are rendered filing contents, often entire narrative blocks.
            # Only remove their markup, never the words being measured.
            tag.unwrap()
        elif name.startswith(_XBRL_PREFIXES) or name in ("ix", "xbrl"):
            tag.decompose()
